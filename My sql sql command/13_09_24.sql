...
select * from customers where state = 'CA' or  city = 'San Francisco';


/*IN operators */
select * from customers where state in ('CA','NY','Victoria');

/*Range Operator - Between AND */

select * from customers where customerNumber between 201 and 299;
...
/*starts with 'SU'*/
select * from customers where contactFirstName like 'SU%';
select contactFirstName from customers where contactFirstName like 'SU%';

/*starts with 'Sue'*/
select * from customers where contactFirstName like 'Sue%';
select contactFirstName from customers where contactFirstName like 'Sue%';


/* end with a */
select * from customers where contactFirstName like '%a';
select contactFirstName from customers where contactFirstName like '%a';

/* contains with ia */
select * from customers where contactFirstName like '%ia%';
select contactFirstName from customers where contactFirstName like '%ia%';

/* 2nd character should be a */

select contactFirstName from customers where contactFirstName like '_a%';
select * from customers where contactFirstName like '_a%';


/*third last character should be 's' */

select contactFirstName from customers where contactFirstName like'%s__';
select * from customers where contactFirstName like'%s__';

/* return name where lenght = 5 or a fixed no of characters*/
...
/*alias clause */
select min(creditLimit) as Minimum_credit from customers;
select max(creditLimit) as Maximum_credit from customers;
...
/*order by clause */
select country,creditLimit from customers order by country;

/*group by clause */
select country from customers group by country;

select * from customers group by country;  /*error degaaaa */

select sum(creditLimit) from customers group by country;

select country,sum(creditLimit) from customers group by country;

select country,state,sum(creditLimit) from customers
	group by country,state order by country;
...
/*Phele where clause chalega then tera group by chalega 
then if agar ussme condition rahegi (having clause) to vo chalega 
then sort krke dene ke liye tera order by chalega */

/*order by meh more than one column liya
 to phele vo 1st column check karega agar 1 se zayda keyword/Naam same aaya 
 to hi next column ke liye order by chalega 
 agar similar val nahi mile jb 1st column ke liye order by chalaya to next wale ke liye chalega hi nahi*/
select country,state,sum(creditLimit) from customers
where creditLimit !=0
group by country,state
having sum(creditLimit) >=100000
...
select customerName,creditLimit from customers where creditLimit= min(credit);  /* this will give error 
																				kyuki min ek tuple return karega 
																				and = sirf val compare karta hai*/



/*sub query with '=' operator */
/* list minimum creditlimit with customer details*/

select customerName,creditLimit from customers where creditLimit = (select min(creditLimit) from customers);

/*list maximum creditlimit with customer details */
select customerName,creditLimit from customers where creditLimit = (select max(creditLimit) from customers);

/*subquery with 'IN' operator */
select customerName,creditLimit from customers where creditLimit in (select min(creditLimit) from customers);

/*list customer details with only order number 10100 or 10101*/
select customerName,customerNumber from customers
where customerNumber = 
(select customerNumber from orders where orderNumber = 10100 or orderNumber = 10101);
...
select customerName,customerNumber from customers
where customerNumber in 
(select customerNumber from orders where orderNumber in (10100,10101));


/*not possible using sub query */
select customerName,customerNumber,orderNumber from customers 
where customerNumber in 
(select customerNumber from orders where
orderNumber in (10100,1010));


/*using join*/
/*list customer details with only order number 10100 or 10101 */
select customerName,o.customerNumber,orderNumber 
from customers c join orders o
on c.customerNumber = o.customerNumber
and orderNumber in (10100,10101);

/*list customer details who done the payment  */
...
/*this only wworks for mySQL and is not universal*/
select * from customers join payments where customers.customernumber = payments.customerNumber;
/*always use this syntax - join with 'on' keyword*/

select * from customers join payments on customers.customernumber = payments.customerNumber;
